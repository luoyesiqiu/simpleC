...
JNIEXPORT void JNICALL Java_jackpal_term_TermExec_sendSignal(JNIEnv *env, jobject clazz,
    jint procId, jint signal)
{
    kill(procId, signal);
}

JNIEXPORT jint JNICALL Java_jackpal_term_TermExec_waitFor(JNIEnv *env, jclass clazz, jint procId) {
    int status;
...
JNIEXPORT jint JNICALL Java_jackpal_term_TermExec_createSubprocessInternal(JNIEnv *env, jclass clazz,
    jstring cmd, jobjectArray args, jobjectArray envVars, jint masterFd)
{
    const jchar* str = cmd ? env->GetStringCritical(cmd, 0) : 0;
    String8 cmd_8;
    if (str) {
        cmd_8.set((char16_t*)str, env->GetStringLength(cmd));
    ...
    if (size > 0) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            tmp_8.set((char16_t*)str, env->GetStringLength(arg));
    ...
    if (size > 0) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            tmp_8.set((char16_t*)str, env->GetStringLength(var));
