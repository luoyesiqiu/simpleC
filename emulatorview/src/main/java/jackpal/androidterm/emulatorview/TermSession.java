...
public class TermSession {
    ...
    public void updateSize(int columns, int rows) {
        if (mEmulator == null) {
            if (EmulatorDebug.DEBUG) {
                Log.d(EmulatorDebug.LOG_TAG, "mEmulator == null" + ",col:" + columns + "," + "rows:" + rows);
            }
    ...
    private void printHexTable(boolean debug, byte[] data) {
        ...
        StringBuilder hexTable = new StringBuilder();
        hexTable.append(" \n");
        StringBuilder hexPart = new StringBuilder();
        StringBuilder asciiPart = new StringBuilder();
        for (int i = 0; i < data.length; i++) {
            char ch = data[i] <= 32 ? '.' : (char) data[i];
            ...
            asciiPart.append(ch);
            if ((i + 1) % 16 == 0) {
                hexTable.append(hexPart.toString());
                hexTable.append("\t");
                hexTable.append(asciiPart.toString());
                hexTable.append("\n");
                hexPart.delete(0, hexPart.capacity());
                asciiPart.delete(0, hexPart.capacity());
            }
        }

        Log.d(EmulatorDebug.LOG_TAG, hexTable.toString());
    }
