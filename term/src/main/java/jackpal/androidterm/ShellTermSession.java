...
public class ShellTermSession extends GenericTermSession {
    ...
    public ShellTermSession(TermSettings settings, String initialCommand) throws IOException {
        ...
        super(ParcelFileDescriptor.open(new File("/dev/ptmx"), ParcelFileDescriptor.MODE_READ_WRITE),
                ...
                settings, true);
        if(TermDebug.DEBUG) {
            Log.d(TermDebug.LOG_TAG, "ShellTermSession");
        }
        initializeSession();
        ...
        mWatcherThread = new Thread() {
            ...
            public void run() {
                ...
                setProcessExitMessage(String.format("Process exited,took %fs.", (mEnd - mStart) / 1000.0));
                try {
                    Thread.sleep(100);
    ...
    public void initializeEmulator(int columns, int rows) {
        super.initializeEmulator(columns, rows);
         if(TermDebug.DEBUG) {
             Log.d(TermDebug.LOG_TAG, "initializeEmulator*");
         }
    ...
    private void sendInitialCommand(String initialCommand) {
        if(TermDebug.DEBUG) {
            Log.d(TermDebug.LOG_TAG, "sendInitialCommand" + ":" + initialCommand);
        }
